...
user_scopes = SecurityScopes(['catalogue', ])
...
class SetupUser:
    ...
    def hashed_password(self):
        return dependencies.hash_password(self.password)


@pytest.fixture
def test_user() -> SetupUser:
    test_user = SetupUser(
        username='user_hashed_password',
        password='unhashed_test_password',
        scopes=['catalogue', ],
    )
    users.add_user(
    ...
    yield test_user
    users.delete_user(test_user.username)


@pytest.fixture
def test_su() -> SetupUser:
    test_user = SetupUser(
        username='su_hashed_password',
        password='su_unhashed_test_password',
        scopes=['catalogue', 'user_manager'],
        su=True,
    )
    users.add_user(
        username=test_user.username,
...
def test_get_current_user(test_user):
    ...
    user = dependencies.get_current_user(user_scopes, tkn)
...
def test_get_current_user_expired(test_user):
    ...
    with pytest.raises(HTTPException):
        dependencies.get_current_user(user_scopes, tkn)
...
def test_get_current_user_compromized_token(test_user):
    ...
    with pytest.raises(HTTPException):
        dependencies.get_current_user(user_scopes, compromized_token)
...
def test_login(test_user):
    ...
    authenticated_user = dependencies.get_current_user(user_scopes, response.json()['access_token'])
